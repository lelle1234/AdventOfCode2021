...
--	select power(2, 4-n) as bitval
        select power(2, 11-n) as bitval
	     , sum(int(substr(s,n+1,1))) > count(1)/2 as sig
	from t 
...
select gamma*eps from (
	select sum(case when sig then bitval end) as gamma
	     , sum(case when not sig then bitval end) as eps 
        from res
);
